...
# Assuming you have a knowledge base implementation for technical scoring
from vector_database import VeterinaryKnowledgeBase
import config


class UnifiedTranscriptScorer:
    def __init__(self, api_key: str, openai_api_key: str = None, model: str = "claude-3-5-sonnet-latest"):
        ...
        # Initialize knowledge base for technical assessment
        try:
    ...
    def normalize_technical_score(self, tech_result: dict) -> dict:
        ...
        for k, v in tech_result.get("parameters", {}).items():
            ...
            explanation += "Strengths:\n- " + "\n- ".join(strengths) if strengths else "Strengths: None identified"
            explanation += "\n\nShortcomings:\n- " + "\n- ".join(shortcomings) if shortcomings else "\n\nShortcomings: None identified"
            ...
            medicine_params = ["Medicine Name", "Medicine Prescribed", "Medicine Dosage"]
            if k in medicine_params:
                improvements_text = comparative_summary if comparative_summary else "No comparative summary available"
            else:
        ...
        normalized = {
            ...
            "total_score": tech_result.get("total_score", 0),  # Changed from overall_score to total_score
            "max_possible_score": tech_result.get("max_possible_score", 0),  # Use actual max_possible_score
            "percentage_score": tech_result.get("overall_score", 0),  # overall_score is already a percentage
    ...
    def score_transcript(self, transcript: str) -> Dict[str, Any]:
        ...
        try:
            print("Scoring Category 2: Pet Information...")
        ...
        try:
            print("Scoring Category 3: Communication Quality...")
        ...
        try:
            print("Scoring Category 4: Technical Assessment...")
        ...
        try:
            print("Scoring Category 5: Call Conclusion...")
        ...
        if results["overall"]["max_possible_score"] > 0:
            results["overall"]["percentage_score"] = round(
                results["overall"]["total_score"] / results["overall"]["max_possible_score"] * 100, 2
            )
        ...
        if "technical_assessment" in results["categories"] and "medicine_findings" in results["categories"]["technical_assessment"]:
            results["medicine_findings"] = results["categories"]["technical_assessment"]["medicine_findings"]
    ...
    def _generate_overall_summary(self, results: Dict[str, Any]) -> str:
        ...
        # Construct summary
        summary = f"Overall call quality is {quality} with a score of {percentage:.1f}%."
        if strengths:
...
def main():
    ...
    parser.add_argument('--output', '-o', help='Output JSON file for results', default='call_scores.json')
    # parser.add_argument('--excel', '-e', help='Output Excel file for report', default='scoring_report.xlsx')
    parser.add_argument('--model', help='LLM model to use', default="claude-3-5-sonnet-latest")
    ...
    if not openai_api_key:
        print("⚠️ Warning: OPENAI_API_KEY environment variable not set. Category 4 scoring may be limited.")

    # Initialize scorer
    scorer = UnifiedTranscriptScorer(api_key=api_key, openai_api_key=openai_api_key, model=args.model)
    ...
    print(f"⏱️  Total execution time: {total_script_time:.2f} seconds ({total_script_time/60:.2f} minutes)")
    # Save JSON results
    ...
    print(f"Total Score: {results['overall']['total_score']} / {results['overall']['max_possible_score']}")
