...
from vector_database import VeterinaryKnowledgeBase
...
class UltraOptimizedTechnicalAssessmentScorer:
    ...
    def __init__(self, knowledge_base: Optional[VeterinaryKnowledgeBase] = None, 
    ...
    def _extract_technical_parameters(self) -> Dict[str, float]:
        ...
        target_params = [
            "Diet Confirmation", "Food Brand Name", "Technical Parameters",
            "Treatment Plan and Instructions PSP", "Medicine Name",
            "Medicine Prescribed", "Medicine Dosage"
        ]
        
        for param in all_parameters:
            param_name = param.get("name")
            if param_name in target_params or param_name in [p.lower() for p in target_params]:
        ...
        if not technical_params:
            technical_params = {
                "Diet Confirmation": 0.03, "Food Brand Name": 0.03,
                "Technical Parameters": 0.04, "Treatment Plan and Instructions PSP": 0.03,
                "Medicine Name": 0.04, "Medicine Prescribed": 0.04, "Medicine Dosage": 0.04
            }
    ...
    def _get_parameter_config(self, parameter_name: str) -> Dict:
        ...
        "Treatment Plan and Instructions PSP" if "treatment plan" in parameter_name.lower() else parameter_name
        ]
        ...
        for param in self.config.get("parameters", []):
            ...
            if parameter_name in config_variants or parameter_name.lower() in [v.lower() for v in config_variants]:
                return param
        ...
        for param_key, max_score in default_max_scores.items():
            if param_key.lower() in parameter_name.lower() or parameter_name.lower() in param_key.lower():
        ...
        return {
            ...
            "subparameters": [{"name": f"{parameter_name} assessment", "weight": correct_max_score}]
        }
    ...
    def _call_llm(self, prompt: str, response_format: str = "json_object") -> Dict:
        ...
        try:
            response = self.openai_client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are a veterinary quality assessment expert."},
        ...
        try:
            ...
            for query in all_queries:
                ...
                for j, result in enumerate(results):
                    print(f"  Result {j+1}: {result['text'][:100]}...")  # First 100 chars
            ...
            for result in combined_results:
                if result["text"] not in seen_texts and len(unique_results) < 12:  # Max 12 knowledge chunks
    ...
    def _mega_knowledge_comparison(self, analysis_result: Dict, knowledge: List[Dict]) -> Dict:
        """
        ...
        for param in self.kb_comparison_params:
            param_analysis = analysis_result.get("parameters_analysis", {}).get(param, {})
            medicine_data[param] = {
        ...
        if pet_details:
            ...
            if pet_details.get("medications"):  
                pet_context += f"- Current medications: {', '.join(pet_details['medications'])}\n"
        
        # Get explicitly mentioned medications for focused comparison
        explicitly_mentioned_meds = analysis_result.get("explicitly_mentioned_medications", [])
        ...
        for param, data in medicine_data.items():
            if data["segments"]:
                ...
                medicine_segments_text += f"Extracted Entities: {', '.join(data['entities'])}\n"
    ...
    def score_technical_assessment(self, transcript: str) -> Dict:
        ...
        if self.kb:
            ...
            if knowledge_queries:
                print("Making LLM Call 2/3: Retrieving knowledge and generating comparisons...")
                knowledge = self._retrieve_knowledge_batch(knowledge_queries)
                if knowledge:
                    knowledge_comparison = self._mega_knowledge_comparison(mega_analysis, knowledge)
                else:
        ...
        def find_matching_param_analysis(target_param_name, analysis_dict):
            ...
            target_normalized = target_param_name.lower().replace(" and ", " ").replace("psp", "").strip()
            for param_name, param_data in analysis_dict.items():
                param_normalized = param_name.lower().replace(" and ", " ").replace("psp", "").strip()
                if target_normalized in param_normalized or param_normalized in target_normalized:
                    return param_data
        ...
        for parameter, weight in self.technical_parameters.items():
            ...
            param_analysis = find_matching_param_analysis(parameter, parameters_analysis)
            ...
            improvements = param_analysis.get("improvements", []) if parameter not in self.kb_comparison_params else []
            raw_percentage = (raw_score / 5) * 100  # LLM always scores 0-5, convert to percentage  
            ...
            comparative_summary = medicine_comparisons.get(parameter) if parameter in self.kb_comparison_params else None
            
            # Determine method and confidence
            method = "lenient pharmacy assessment" if parameter in self.pharmacy_params else "direct LLM assessment"
            confidence = "High" if len(segments) >= 2 and raw_score > 3.5 else \
                        "Medium" if len(segments) >= 1 and raw_score > 2.0 else "Low"
            ...
            results[parameter] = {
                ...
                "method": method,
                "strengths": strengths,
                "shortcomings": shortcomings if shortcomings else ["No specific shortcomings identified."],
                "improvements": improvements if parameter not in self.kb_comparison_params else [],
        ...
        total_max_score = sum(param_config["max_score"] for param_config in [self._get_parameter_config(p) for p in self.technical_parameters.keys()])
        actual_total_score = sum(result["score"] for result in results.values())
        category_percentage = (actual_total_score / total_max_score) * 100 if total_max_score > 0 else 0
        # Get summaries from mega analysis
        global_summary = mega_analysis.get("global_summary", "Technical assessment completed.")
        medicine_summary = mega_analysis.get("medicine_findings_summary", "Medicine analysis completed.")
